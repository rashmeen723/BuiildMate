...
export class ServicesService implements OnModuleInit {
    ...
    async updateBookingStatus(bookingId: string, status: string, additionalCharges: number = 0, reason?: string, cancelledBy?: string) {
        ...
        if (status === 'COMPLETED') {
            ...
            if (booking && booking.arrivedAt) {
                const durationMs = new Date().getTime() - new Date(booking.arrivedAt).getTime();
                const durationHours = durationMs / (1000 * 60 * 60);
                const hourlyRate = booking.provider?.serviceProvider?.hourlyRate || 500;

                // Calculate base fee (minimum 1 hour charge)
                const baseAmount = Math.max(1, durationHours) * hourlyRate;
                const totalBeforePlatformFee = baseAmount + additionalCharges;

                // Add a 5% App Service Fee
                const appServiceFee = totalBeforePlatformFee * 0.05;
                const finalAmount = totalBeforePlatformFee + appServiceFee;
